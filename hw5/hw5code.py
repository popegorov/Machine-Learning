...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node):
        ...
        for feature in range(sub_X.shape[1]): #
            ...
            if len(feature_vector) == 1 or np.all(feature_vector == feature_vector[0]): #
                continue
    ...
    def _predict_node(self, x, node):
        ...
        if self._feature_types[node['feature_split']] == 'real' and x[node['feature_split']] < node['threshold']:
            return self._predict_node(x, node['left_child'])
        if self._feature_types[node['feature_split']] == 'categorical' and x[node['feature_split']] in node['categories_split']:
            return self._predict_node(x, node['left_child'])
